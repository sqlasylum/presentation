#!/bin/bash
# Nightly FULL backups for multiple PostgreSQL servers using pgBackRest.
# Intended schedule example (daily at 01:30):
...
echo "[$RUN_TS] Starting nightly FULL backups for ${#STANZAS[@]} stanza(s)."
...
for STANZA in "${STANZAS[@]}"; do
  ...
  echo "[$START_TS] Running FULL backup for stanza '$STANZA'"
  "$PGBACKREST_BIN" --stanza="$STANZA" --type=full backup

  END_TS="$(date '+%Y-%m-%d %H:%M:%S')"
  echo "[$END_TS] FULL backup completed for stanza '$STANZA'"
done

DONE_TS="$(date '+%Y-%m-%d %H:%M:%S')"
echo "[$DONE_TS] All nightly FULL backups completed successfully."
